...
@Injectable()
export class PnrAgencyService {
  constructor(
    private pnrRemoteRepository: PnrAgencyRemoteRepository,
  ...
  async summaryPnr(request: SummaryPnrAgencyRequest, user: string) {
    ...
    request = plainToInstance(SummaryPnrAgencyRequest, { ...request, segments }) as SummaryPnrAgencyRequest;

    const summaryResponse = await this.pnrRemoteRepository.summaryPnr(request, request.token, user);
  ...
  async createPnrAgencyWeb(request: CreatePnrAgencyWebRequest, user: string) {
