...
@Injectable()
export class PaymentsAgencyService {
  constructor(
    private readonly pnrRemoteRepository: PnrAgencyRemoteRepository,
    ...
    private readonly reservationRemoteRepository: ReservationAgencyRemoteRepository,
  ...
  /* async generateCheckoutUrl(paymentInput: EpgPaymentInputAgencyDto, user: string): Promise<any> {
    const token = paymentInput.token;
  ...
  async savePayment(paymentInput: EpgPaymentInputDto, token: string, iata: string, user: string): Promise<any> {
    /* const customerId = Date.now().toString(); */
  ...
  async paywithcreditagency(paymentInput: PaymentWithAgencyCreditDto, user: string): Promise<any> {
    const token = paymentInput.token;
    /* 
    let confirmationNumber = paymentInput.confirmationNumber;
    let pnrData;
    if (confirmationNumber == null) {
      ...
      const addApisInfoStart = Date.now();
      const payload = this.apisInfoRemoteRepository.generatePayloadDataAgency(paymentInput, pnrData);
      //API DOCS
    ...
    //una ves creada la reserva.
    //genero el pago con credito de la agencia
    paymentInput.confirmationNumber = confirmationNumber;
    */
    const ProcessPNRPaymentResponse = await this.paymentRemoteRepository.ProcessPNRPayment(paymentInput);
    const retrievePnrException = await this.jsonLogger.processException(
      ProcessPNRPaymentResponse,
      'ProcessPNRPaymentResponse',
      'ProcessPNRPaymentResult'
    );
    if (retrievePnrException.getStatus() !== StatusCode.SUCCESS) {
      this.statsdService.timing('_radixx_error', 4000);
      return Promise.reject(retrievePnrException);
