...
@Injectable()
export class SeatsAgencyService {
  ...
  private logger = new JsonLogger(SeatsAgencyService.name);
  constructor(
  ...
  public async getSeatsRemote(seatsInputDto: SeatsInputDto, token: string, user: string) {
  ...
  public async getSeatList(seatsInputDto: SeatsInputAgencyDto, user: string): Promise<GetSeatListResponse> {
    const { fareBundle, serviceBundle, airportCode, destinationAirportCode } = seatsInputDto;
    const token = seatsInputDto.token;
  ...
  public async editSeats(editSeatDto: EditSeatDto, token: string, user: string) {
