...
@Injectable()
export class FlightsAgencyService {
  constructor(
  ...
  async retrieveAllFlights(request: GetAllFlightAgencyRequest, user: string): Promise<OriginAndDestinationsResponse[]> {
    const prismicStart = Date.now();
    const { airports } = await this.prismicService.getAirports(request);
    ...
    const flightsResponse = await this.flightsRemoteRepository.getAllFlights(
  ...
  private availableRoutesDtoResponseMapper(flights: any, airports: AirportResponse[]): OriginAndDestinationsResponse[] {
    const routes =
      flights['s:Body']['RetrieveAirportRoutesResponse']['RetrieveAirportRoutesResult']['a:Routes']['a:Route'];
    ...
    routes.forEach((element) => {
      if (element['a:OriginAirportCode'] === lastOriginProccesed) {
        ...
        if (
          ...
          !airports.find((airport: AirportResponse) => airport.code === element['a:DestinationAirportCode'])
        ) {
