...
@Injectable()
export class ReservationAgencyService {
  constructor(
    private remoteRepository: ReservationAgencyRemoteRepository,
    private prismicRemoteRepository: PrismicRemoteRepository,
    private fareQuotesService: FareQuotesAgencyService,
  ...
  public async retrieveAARQuote(getAARQuote: GetAARQuote, token: string, user: string) {
  ...
  public async getReservationExtras(getReservationExtras: GetReservationExtrasDto, token: string, user: string) {
    const { lang } = getReservationExtras;
    const reservation = await this.retrieveReservation(getReservationExtras, token, false, user);
  ...
  public async getExtras(getReservationExtras: GetExtrasDto, token: string, user: string): Promise<GetExtrasResponse> {
    ...
    const formattedFlights = flights.map(async (flight: FlightOfReservationDto) => {
      ...
      const serviceBundleDetails = serviceBundle
        ? await this.fareQuotesService.getServiceBundleDetails(serviceBundle, token, user)
        : null;
      ...
      const fareBundleDetails = fareBundle
        ? await this.fareQuotesService.getFareBundleDetails(fareBundle, token, user)
        : null;
